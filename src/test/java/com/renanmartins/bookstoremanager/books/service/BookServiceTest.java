...
public class BookServiceTest {
    ...
    void whenExistingBookIdIsInformedThenItShouldBeUpdated() {
        ...
        BookResponseDTO expectedUpdateBookDTO = bookResponseDTOBuilder.buildBookResponse();
        Book expectedUpdateBook = bookMapper.toModel(expectedUpdateBookDTO);

        when(userService.verifyAndGetUserIfExists(authenticatedUser.getUsername())).thenReturn(new User());
        when(bookRepository.findByIdAndUser(eq(expectedBookToUpdateDTO.getId()), any(User.class)))
                .thenReturn(Optional.of(expectedUpdateBook));
        ...
        when(bookRepository.save(any(Book.class))).thenReturn(expectedUpdateBook);
        ...
        assertThat(updatedBookResponseDTO, is(equalTo(expectedUpdateBookDTO)));
    }

    @Test
    void whenNotExistingBookIdIsInformedThenAExceptionShouldBeThrown() {
        BookRequestDTO expectedBookToUpdateDTO = bookRequestDTOBuilder.buildRequestBookDTO();
